...
# Import Phase 3 functions (WORKERS COMPENSATION VERSION)
from A_phase3_llm_worker import read_combined_file, create_chunks, extract_with_llm, merge_extraction_results, save_extraction_results, create_final_validated_fields
    ...
    try:
        ...
        if phase2_results:
            ...
            if pymupdf_pages and ocr_pages:
                ...
                # OPTION 1: No PyMuPDF clean pages, use OCR for all pages
                print(f"No PyMuPDF clean pages for {carrier['name']}, using OCR for all pages")
                phase2c_results = {}
        ...
        if phase2c_results:
            ...
            if ocr_pages_2d:  # Only need OCR pages since PyMuPDF might be empty
                ...
                print(f"Could not read OCR pages for Phase 2D for {carrier['name']}")
...
def push_multi_carrier_to_sheets(carriers):
    ...
    coverage_type = "Workers Compensation Coverages"  # For workers compensation insurance
